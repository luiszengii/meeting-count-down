...
- `SettingsSceneOpenControllerTests.swift`：锁定 SwiftUI `Settings` scene 打开动作桥接的登记、覆盖和调用语义。
...
这里的测试通常不覆盖具体视觉渲染，而是验证“菜单弹层登记了什么动作、壳层控制器会不会走到官方 scene 打开路径、重复挂载时是否还调用最新环境值”这类状态和动作流。
